...
public class RedisServiceTest {
    ...
    public void skalHenteFraCache() throws JsonProcessingException {
        when(redisStore.get("key")).thenReturn(objectMapper.writeValueAsBytes(kommuneInfo));

        KommuneInfo cached = (KommuneInfo) redisService.get("key", KommuneInfo.class);
    ...
    public void skalHenteAlleKommuneInfos() throws JsonProcessingException {
        ...
        when(redisStore.get("key")).thenReturn(bytes);

        Map<String, KommuneInfo> cached = redisService.getKommuneInfos("key");
    ...
    public void ingenKommuneInfos() {
        when(redisStore.get("key")).thenReturn(null);

        Map<String, KommuneInfo> map = redisService.getKommuneInfos("key");
