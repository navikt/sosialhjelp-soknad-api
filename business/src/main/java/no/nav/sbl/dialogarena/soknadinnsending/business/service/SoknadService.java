...
public class SoknadService implements SendSoknadService {
	...
	public Faktum lagreSoknadsFelt(Long soknadId, Faktum faktum) {
		Long faktumId = repository.lagreFaktum(soknadId, new Faktum(soknadId,
				faktum.getFaktumId(), faktum.getKey(), faktum.getValue(),
				BRUKERREGISTRERT_FAKTUM));
	...
	public Long startSoknad(String navSoknadId) {
		...
		WebSoknad soknad = WebSoknad.startSoknad()
				.medBehandlingId(behandlingsId).medGosysId(navSoknadId)
		...
		try {
			...
			if (new IsImage().evaluate(bytes)) {
				if (ScriptRunner.IM_EXISTS) {
					bytes = new ScriptRunner(ScriptRunner.Type.IM,
							IMAGE_RESIZE, null, new ByteArrayInputStream(bytes))
							.call();
					bytes = new ScriptRunner(ScriptRunner.Type.IM, IMAGE_PDFA,
							null, new ByteArrayInputStream(bytes)).call();
				} else {
					bytes = new ImageToPdf().transform(bytes);
				}
			} else if (new IsPdf().evaluate(bytes)) {
				if (ScriptRunner.GS_EXISTS) {
					bytes = new ScriptRunner(ScriptRunner.Type.GS, PDF_PDFA,
							null, new ByteArrayInputStream(bytes)).call();
				}
			}
	...
	public byte[] lagForhandsvisning(Long soknadId, Long vedleggId) {
		try {
			return new ConvertToPng(new Dimension(600, 800),
	...
	public Long genererVedleggFaktum(Long soknadId, Long faktumId) {
		...
		Vedlegg vedlegg = new Vedlegg(null, soknadId, faktumId, "faktum.pdf",
				Long.valueOf(doc.length), doc);
		...
		vedleggRepository.knyttVedleggTilFaktum(soknadId, faktumId,
				opplastetDokument);
