...
public class ArbeidsforholdSystemdata implements Systemdata {
    ...
    private void updateVedleggForventninger(JsonData jsonData) {
        List<JsonOkonomiOpplysningUtbetaling> utbetalinger = jsonData.getOkonomi().getOpplysninger().getUtbetaling();
        List<JsonOkonomioversiktInntekt> inntekter = jsonData.getOkonomi().getOversikt().getInntekt();
        ...
        if (typeIsInList(jsonVedleggs, "sluttoppgjor")){
            String tittel = textService.getJsonOkonomiTittel(jsonTypeToFaktumKey.get(soknadstype));
