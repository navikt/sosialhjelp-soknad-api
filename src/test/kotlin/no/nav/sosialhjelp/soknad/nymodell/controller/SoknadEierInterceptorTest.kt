...
class SoknadEierInterceptorTest {
    ...
    fun`Soknad med riktig eier er ok`() {
        every { soknadRepository.findById(soknadId) } returns Optional.of(Soknad(soknadId, Eier(eier)))
    ...
    fun `Soknad-request med feil soknadId blir stoppet`() {
        ...
        every { soknadRepository.findById(nySoknadId) } returns Optional.of(Soknad(nySoknadId, Eier("12345612345")))
