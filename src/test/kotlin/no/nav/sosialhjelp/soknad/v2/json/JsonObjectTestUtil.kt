    ...
    return json.copy(
        soknad =
            soknad.copy(
                data =
                    soknad.data.copy(
                        personalia = soknad.data.personalia.copy(personIdentifikator = JsonPersonIdentifikator(JsonPersonIdentifikator.Kilde.SYSTEM, eier)),
                        soknadstype = if (kortSoknad) JsonData.Soknadstype.KORT else JsonData.Soknadstype.STANDARD,
                        arbeid = JsonArbeid(),
                        utdanning = JsonUtdanning(BRUKER),
                        bosituasjon = JsonBosituasjon(JsonKildeBruker.BRUKER),
