...
class AntallInnsendteSoknaderIntegrationTest : AbstractIntegrationTest(useTokenX = true) {
    ...
    fun `antallSisteDogn returnerer antall innsendte soknader siste 24 timer`() {
        repeat(3) { sendtSoknadForBruker(sendtInn = LocalDateTime.now().minusHours(12)) }
    ...
    fun `antallSisteDogn inkluderer ikke soknader med status OPPRETTET eller INNSENDING_FEILET`() {
        metadataRepository.save(opprettSoknadMetadata(status = SoknadStatus.OPPRETTET, personId = userId, innsendtDato = LocalDateTime.now().minusHours(1)))
        metadataRepository.save(opprettSoknadMetadata(status = SoknadStatus.INNSENDING_FEILET, personId = userId, innsendtDato = LocalDateTime.now().minusHours(1)))
        sendtSoknadForBruker(sendtInn = LocalDateTime.now().minusHours(1))
    ...
    fun `antallSisteDogn returnerer eldsteSendtInn kun naar antall er 10`() {
        val eldsteTidspunkt = LocalDateTime.now().minusHours(20)
        sendtSoknadForBruker(sendtInn = eldsteTidspunkt)
        repeat(9) { sendtSoknadForBruker(sendtInn = LocalDateTime.now().minusHours(1)) }
    ...
    fun `antallSisteDogn inkluderer ikke soknader for annen bruker`() {
        metadataRepository.save(
            opprettSoknadMetadata(
                ...
                innsendtDato = LocalDateTime.now().minusHours(1),
            ),
        )
        sendtSoknadForBruker(sendtInn = LocalDateTime.now().minusHours(1))
