...
class FilMetaKonverteringTest {
    ...
    fun `Test konverter excel-fil (xlsx) stottes`() {
        val stottetFiltype = finnKonverterer(EXCEL_FILE.readBytes(), EXCEL_FILE.name)
    ...
    fun `Test excel-fil (xls) stottes ikke`() {
        val stottetFiltype = finnKonverterer(
            ExampleFileRepository.EXCEL_FILE_OLD.readBytes(),
    ...
    fun `Test word-fil (docx) stottes`() {
        val stottetFiltype = finnKonverterer(WORD_FILE.readBytes(), WORD_FILE.name)
    ...
    fun `Test word-fil (doc) stottes ikke`() {
        val stottetFiltype = finnKonverterer(
            ExampleFileRepository.WORD_FILE_OLD.readBytes(),
    ...
    fun `Test csv-fil (csv) stottes`() {
        val stottetFiltype = finnKonverterer(CSV_FILE.readBytes(), CSV_FILE.name)
