...
import no.nav.sosialhjelp.soknad.domene.personalia.repository.TelefonnummerRepository
...
class TelefonnummerRepositoryTest: RepositoryTest() {
    ...
    fun `Duplikate id-kolonner skal gi feil`() {
        ...
        val telefonnummer = Telefonnummer(soknad.id, SoknadKilde.SYSTEM, "1234556778")
        telefonnummerRepository.save(telefonnummer)

        val annetTelefonnummer = Telefonnummer(soknad.id, SoknadKilde.SYSTEM, "353422354")
    ...
    fun `Lagre for SYSTEM og BRUKER pa samme soknad`() {
        ...
        val systemNummer = Telefonnummer(soknad.id, SoknadKilde.SYSTEM, "1234556778")
        telefonnummerRepository.save(systemNummer)

        val brukerNummer = Telefonnummer(soknad.id, SoknadKilde.BRUKER, "353422354")
        telefonnummerRepository.save(brukerNummer)

        assertThat(telefonnummerRepository.findAllBySoknadId(soknad.id).size).isEqualTo(2)
    }
    ...
    fun `By design kan man ikke oppdatere telefonnummer - en ma slette og lagre nytt`() {
        ...
        val telefonnummer = Telefonnummer(soknad.id, SoknadKilde.BRUKER, opprinneligNummer).let {
