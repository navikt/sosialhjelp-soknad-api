...
class KontaktToJsonMapper(
    ...
    internal companion object Mapper {
                            ...
                            data.copy(
                                personalia =
                                    personalia.copy(
                                        ...
                                        oppholdsadresse = valg?.let { oppholdsadresse.toJsonAdresse(if (it == AdresseValg.SOKNAD) JsonKilde.BRUKER else JsonKilde.SYSTEM, JsonAdresseValg.fromValue(it.name.lowercase())) },
                                        postadresse = valg?.let { oppholdsadresse.toJsonPostadresse(it) },
                                    ),
                            ),
                        mottaker = kontakt.mottaker?.toJsonSoknadsmottaker() ?: soknad.mottaker,
                    ),
        ...
        private fun Adresse.toJsonPostadresse(valg: AdresseValg): JsonAdresse? = if (this is MatrikkelAdresse) null else toJsonAdresse(if (valg == AdresseValg.SOKNAD) JsonKilde.BRUKER else JsonKilde.SYSTEM)
        ...
        ) = JsonGateAdresse(kilde, landkode, kommunenummer, adresselinjer, bolignummer, postnummer, poststed, gatenavn, husnummer, husbokstav, adresseValg)

        private fun MatrikkelAdresse.toJsonMatrikkelAdresse(
            kilde: JsonKilde,
            adresseValg: JsonAdresseValg?,
        ) = JsonMatrikkelAdresse(kilde, kommunenummer, gaardsnummer, bruksnummer, festenummer, seksjonsnummer, undernummer, adresseValg)

        private fun UstrukturertAdresse.toJsonUstrukturertAdresse(
            kilde: JsonKilde,
            adresseValg: JsonAdresseValg?,
        ) = JsonUstrukturertAdresse(kilde, adresse, adresseValg)
        ...
        private fun NavEnhet.toJsonSoknadsmottaker(): no.nav.sbl.soknadsosialhjelp.soknad.JsonSoknadsmottaker? =
            no.nav.sbl.soknadsosialhjelp.soknad.JsonSoknadsmottaker(kommunenummer, enhetsnummer, "$enhetsnavn, $kommunenavn")
    }
}
