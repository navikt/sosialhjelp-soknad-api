...
object OkonomiskeOpplysningerOgVedleggSteg {
    fun get(
        ...
        val okonomi = requireNotNull(requireNotNull(jsonInternalSoknad.soknad).data.okonomi)
    ...
    private fun vedleggFelter(
        vedlegg: JsonVedlegg,
        vedleggInfo: List<OppsummeringVedleggInfo>,
    ): List<Felt> {
        val felt: Felt =
            if ("LastetOpp" == vedlegg.status && vedlegg.filer != null && vedlegg.filer.isNotEmpty()) {
                Felt(
                    type = Type.VEDLEGG,
                    vedlegg =
                        vedlegg.filer.map {
