...
fun leggTilOkonomiskeOpplysningerOgVedlegg(
    ...
    for (inntekt in okonomi.oversikt.inntekt) {
        ...
        if (inntekt.type == "studielanOgStipend") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, inntekt.netto, "opplysninger.arbeid.student.utbetaling.label")
        }
        if (inntekt.type == "jobb") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, inntekt.brutto, "opplysninger.arbeid.jobb.bruttolonn.label")
        ...
        if (inntekt.type == "barnebidrag") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, inntekt.netto, "opplysninger.familiesituasjon.barnebidrag.mottar.mottar.label")
        }
        pdf.addBlankLine()
    }
    val husbankenUtbetalinger: List<JsonOkonomiOpplysningUtbetaling> = hentUtbetalinger(okonomi, UTBETALING_HUSBANKEN)
    for (husbankenUtbetaling in husbankenUtbetalinger) {
        if (husbankenUtbetaling.kilde == JsonKilde.BRUKER) {
            ...
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, husbankenUtbetaling.netto, "opplysninger.inntekt.bostotte.utbetaling.label")
            pdf.addBlankLine()
        }
    }
    ...
    for (formue in okonomi.oversikt.formue) {
        if (sparingTyper.contains(formue.type)) {
            ...
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, formue.belop, "opplysninger.inntekt.bankinnskudd." + formue.type + ".saldo.label")
            pdf.addBlankLine()
        }
    }
    ...
    for (utbetaling in okonomi.opplysninger.utbetaling) {
        if (utbetaling.type != "skatteetaten" && utbetaling.type != "navytelse" && utbetaling.type != "husbanken") {
            ...
            if (utbetaling.type == "sluttoppgjoer") {
                pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utbetaling.belop, "opplysninger.arbeid.avsluttet.netto.label")
            } else {
                pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utbetaling.belop, "opplysninger.inntekt.inntekter." + utbetaling.type + ".sum.label")
            }
            pdf.addBlankLine()
        }
    }
    ...
    for (utgift in okonomi.opplysninger.utgift) {
        ...
        if (utgifterBarnAlternativer.contains(utgift.type)) {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.barn." + utgift.type + ".sisteregning.label")
        }
        if (boutgiftAlternativer.contains(utgift.type)) {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.boutgift." + utgift.type + ".sisteregning.label")
        }
        if (utgift.type == "annen") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.ekstrainfo.utgifter.utgift.label")
        }
        pdf.addBlankLine()
    }
    for (utgift in okonomi.oversikt.utgift) {
        pdf.skrivTekst(utgift.tittel)
        if (utgifterBarnAlternativer.contains(utgift.type)) {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.barn." + utgift.type + ".sistemnd.label")
        }
        if (utgift.type == "barnebidrag") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.familiesituasjon.barnebidrag.betaler.betaler.label")
        }
        if (utgift.type == "husleie") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.boutgift.husleie.permnd.label")
        }
        if (utgift.type == "boliglanAvdrag") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.boutgift.avdraglaan.avdrag.label")
        }
        if (utgift.type == "boliglanRenter") {
            pdfUtils.skrivTekstMedGuardOgIkkeUtfylt(pdf, utgift.belop, "opplysninger.utgifter.boutgift.avdraglaan.renter.label")
        }
        pdf.addBlankLine()
    }
