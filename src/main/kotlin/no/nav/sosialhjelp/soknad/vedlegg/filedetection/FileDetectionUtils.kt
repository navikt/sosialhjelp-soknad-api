...
object FileDetectionUtils {
    ...
    fun mapToTikaType(mimeType: String): TikaFileType =
        when (mimeType) {
            MimeTypes.APPLICATION_PDF -> TikaFileType.PDF
            MimeTypes.TEXT_X_MATLAB -> {
                log.info(
                    "Tika detekterte mimeType text/x-matlab. Vi antar at dette egentlig er en PDF, men som ikke har korrekte magic bytes (%PDF).",
                )
                TikaFileType.PDF
            }
