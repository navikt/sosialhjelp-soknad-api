...
class InntektUtgiftFormueMapper(
    ...
    override fun mapToSoknadJson(soknadId: UUID, jsonInternalSoknad: JsonInternalSoknad) {
        ...
        val okonomiBubbleObjects = listOf(inntekterForSoknad, utgifterForSoknad, formueForSoknad).flatten()

        JsonOkonomi().apply { mapFromDomainObjects(okonomiBubbleObjects) }
    }
}

fun JsonOkonomi.mapFromDomainObjects(okonomiObjects: List<OkonomiBubbleObject>) {
    okonomiObjects.forEach { mapFromDomainObject(it) }
}
