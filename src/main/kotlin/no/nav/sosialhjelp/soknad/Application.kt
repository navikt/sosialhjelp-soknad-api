...
open class Application {
    ...
    companion object {
        ...
        private fun configureApplication(builder: SpringApplicationBuilder): SpringApplicationBuilder {
            if (!MiljoUtils.isNonProduction() && MockUtils.isMockAltProfil()) {
                throw Error("mockAltProfil har blitt satt til true i prod. Stopper applikasjonen da dette er en sikkerhetsrisiko.")
            }
            if (!MiljoUtils.isNonProduction() && MockUtils.isRunningWithInMemoryDb()) {
            ...
            if (!MiljoUtils.isNonProduction() && (MockUtils.isAlltidHentKommuneInfoFraNavTestkommune() || MockUtils.isAlltidSendTilNavTestkommune())) {
