...
public class SoknadComponent extends WebComponent {
    ...
    public void onComponentTagBody(MarkupStream markupStream, ComponentTag openTag) {
        ...
        if (RuntimeConfigurationType.DEVELOPMENT.equals(getApplication().getConfigurationType())) {
            bootstrapDev(markupStream, openTag);
        } else {
            bootstrapProd(markupStream, openTag);
        }
    }

    private void bootstrapDev(MarkupStream markupStream, ComponentTag openTag) {
        try {
            ...
            InputStream content = new FileInputStream(devDir);
            replaceComponentTagBody(markupStream, openTag, copyToString(content, forName("UTF-8")));
        } catch (IOException |URISyntaxException e) {
            logger.warn("Problem med ressurslasting" + e);
        }
    }

    private void bootstrapProd(MarkupStream markupStream, ComponentTag openTag) {
