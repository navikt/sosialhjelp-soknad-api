package no.nav.sbl.dialogarena.websoknad.common;

import no.nav.modig.core.exception.ApplicationException;
import no.nav.sbl.dialogarena.websoknad.domain.WebSoknad;
import no.nav.sbl.dialogarena.websoknad.pages.basepage.BasePage;
import no.nav.sbl.dialogarena.websoknad.pages.tullesoknad.persondata.TullesoknadPersondataPage;
...
public enum PageEnum {
    ...
    public static Page getPage(WebSoknad soknad) {
        ...
        for (PageEnum page : values()) {
            if (page.erSide(soknad.getGosysId())) {
                try {
                    return page.pageClass.getConstructor(WebSoknad.class).newInstance(soknad);
