...
public class VedleggControllerTest {
    ...
    public void shouldUploadFile() throws Exception {
        ...
        when(vedleggService.hentVedlegg(eq(11L), eq(1L), eq(false))).thenReturn(new Vedlegg(1L, 11L, 12L, "0", "L6", "test", 1L, 1, "gfdg", null, Vedlegg.Status.VedleggKreves));
        when(vedleggService.hentVedlegg(eq(11L), eq(2L), eq(false))).thenReturn(new Vedlegg(2L, 11L, 12L, "0", "L6", "test", 1L, 1, "gfdg", null, Vedlegg.Status.VedleggKreves));
        when(vedleggService.hentVedlegg(eq(11L), eq(3L), eq(false))).thenReturn(new Vedlegg(3L, 11L, 12L, "0", "L6", "test", 1L, 1, "gfdg", null, Vedlegg.Status.VedleggKreves));
    ...
    public void skalGenereFerdigPdf() throws Exception {
        Vedlegg v = new Vedlegg(1L, 11L, 1L, "0", "1", "", 3L, 1, null, null, Vedlegg.Status.VedleggKreves);
