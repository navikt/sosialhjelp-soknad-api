...
public class SoknadBekreftelseControllerTest {
    ...
    public void skalSendeEpost() throws Exception {
        ...
        verify(emailService).sendEpostEtterInnsendtSoknad("test@epost.com", "emne", "<p>Tekst. Saksoversikturl null/detaljer/DAG/123 og ettersendelseurl http://localhost:80/bekreftelse/123/startettersending/123</p>", "123");
    }
    ...
    public void skalSendeEpostMedEttersendelseInnhold() throws Exception {
        ...
        mockMvc.perform(post("/bekreftelse/{behandlingId}", "123").contentType(MediaType.APPLICATION_JSON).content("{\"epost\": \"test@epost.com\", \"temaKode\": \"DAG\", \"erEttersendelse\": true}"))
                .andExpect(status().isOk());
        verify(emailService).sendEpostEtterInnsendtSoknad("test@epost.com", "emne", "<p>Ettersendelse. Saksoversikturl null/detaljer/DAG/123</p>", "123");
    }
}
