...
function rsint64(buffer, endian, offset)
{
	...
	/*
	 * If we had the key 0x8***REMOVED***0000, that would leave the lower 32
	 * bits as 0xffffffff, however, since we're goint to add one, that would
	 * actually leave the lower 32-bits as 0***REMOVED***000, which would break
	 ...
	 */
	mod_assert.ok(val[1] <= 0***REMOVED***000);
	if (val[1] == -0***REMOVED***000) {
		val[1] = 0;
		val[0]--;
	}
...
function wsint32(value, endian, buffer, offset)
{
	...
	val = prepsint(value, 0x7fffffff, -0***REMOVED***00);
	if (val >= 0)
		wgint32(val, endian, buffer, offset);
	else
...
function wsint64(value, endian, buffer, offset)
{
	...
	if (vzpos) {
		...
		prepsint(value[0], 0, -0***REMOVED***00);
		prepsint(value[1], 0, -0xffffffff);
		if (value[0] == -0***REMOVED***00 && value[1] != 0)
	...
	if (value[0] < 0 || value[1] < 0) {
		...
		vals[1] = 0***REMOVED***000 - Math.abs(value[1]);
		if (vals[1] == 0***REMOVED***000) {
			vals[1] = 0;
			vals[0]++;
		}
	} else {
