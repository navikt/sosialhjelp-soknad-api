/**
 * CBCModeTest
 * 
 * A test class for CBCMode
 * Copyright (c) 2007 Henri Torgemane
 * 
 * See LICENSE.txt for full license information.
 */
package com.hurlant.crypto.tests
{
	import com.hurlant.crypto.symmetric.AESKey;
	import com.hurlant.crypto.symmetric.CBCMode;
	import com.hurlant.crypto.symmetric.NullPad;
	import com.hurlant.crypto.symmetric.XTeaKey;
	import com.hurlant.util.Hex;
	
	import flash.utils.ByteArray;
	
	public class CBCModeTest extends TestCase
	{
		public function CBCModeTest(h:ITestHarness) {
			super(h, "CBCMode Test");
			runTest(testAES,"CBC AES Test Vectors");
			runTest(testXTea,"CBC XTea Test Vectors");
			//
			runTest(testCBC_AES128, "CBC AES-128 Test Vectors");
			runTest(testCBC_AES192, "CBC AES-192 Test Vectors");
			runTest(testCBC_AES256, "CBC AES-256 Test Vectors");
			h.endTestCase();
		}
		
		/**
		 * Hawt NIST Vectors: http://csrc.nist.gov/publications/nistpubs/800-38a/sp800-38a.pdf
		 * Section F.2.1 and below.
		 */
		public function testCBC_AES128():void {
			var key:ByteArray = Hex.toArray("2b7***REMOVED***aed2a6ab***REMOVED***9cf4f3c");
			var pt:ByteArray = Hex.toArray(
				"6bc1bee22e409f96e93d7***REMOVED***172a" + 
				"ae2d8a571e03ac9c9eb76fac45af8e51" + 
				"30c81c46a35ce411e5fbc1191a0a52ef" + 
				"f69f2445df4f9b17ad2b417be66c3710");
			var ct:ByteArray = Hex.toArray(
				"7649abac8119b246cee98e9b12e9197d" + 
				"5086cb9***REMOVED***ee95db113***REMOVED***b2" + 
				"73bed6b8e3c1743b7116e69***REMOVED***16" + 
				"3ff1caa1681fac09120ec***REMOVED***e1a7");
			var cbc:CBCMode = new CBCMode(new AESKey(key), new NullPad);
			cbc.IV = Hex.toArray("***REMOVED***5060708090a0b0c0d0e0f");
			var src:ByteArray = new ByteArray;
			src.writeBytes(pt);
			cbc.encrypt(src);
			assert("CBC_AES128 test 1", Hex.fromArray(src)==Hex.fromArray(ct));
			cbc.decrypt(src);
			assert("CBC_AES128 test 2", Hex.fromArray(src)==Hex.fromArray(pt));
		}
		public function testCBC_AES192():void {
			var key:ByteArray = Hex.toArray("8e73b0f7da0e6452c810f32***REMOVED***e562f8ead2522c6b7b");
			var pt:ByteArray = Hex.toArray(
				"6bc1bee22e409f96e93d7***REMOVED***172a" + 
				"ae2d8a571e03ac9c9eb76fac45af8e51" + 
				"30c81c46a35ce411e5fbc1191a0a52ef" + 
				"f69f2445df4f9b17ad2b417be66c3710");
			var ct:ByteArray = Hex.toArray(
				"4f021db243bc633***REMOVED***3a9fa071e8" + 
				"b4d9ada9ad7dedf4e5***REMOVED***f69145a" + 
				"571***REMOVED***fb7ae07fa9baac3df102e0" + 
				"08b0***REMOVED***98881d920a9e64f5615cd");
			var cbc:CBCMode = new CBCMode(new AESKey(key), new NullPad);
			cbc.IV = Hex.toArray("***REMOVED***5060708090a0b0c0d0e0f");
			var src:ByteArray = new ByteArray;
			src.writeBytes(pt);
			cbc.encrypt(src);
			assert("CBC_AES192 test 1", Hex.fromArray(src)==Hex.fromArray(ct));
			cbc.decrypt(src);
			assert("CBC_AES192 test 2", Hex.fromArray(src)==Hex.fromArray(pt));
		}
		public function testCBC_AES256():void {
			var key:ByteArray = Hex.toArray(
				"603deb1015ca71be2b73aef0857d7781" + 
				"1f352c073b6108d72d9810a30914dff4");
			var pt:ByteArray = Hex.toArray(
				"6bc1bee22e409f96e93d7***REMOVED***172a" + 
				"ae2d8a571e03ac9c9eb76fac45af8e51" + 
				"30c81c46a35ce411e5fbc1191a0a52ef" + 
				"f69f2445df4f9b17ad2b417be66c3710");
			var ct:ByteArray = Hex.toArray(
				"f58c4c04d6e5f1ba779eabfb5f7bfbd6" + 
				"9cfc4e967edb808d679f777bc6702c7d" + 
				"39f23369a9d9bacfa530***REMOVED***31461" + 
				"b2eb05e2c39be9fcda6c19078c6a9d1b");
			var cbc:CBCMode = new CBCMode(new AESKey(key), new NullPad);
			cbc.IV = Hex.toArray("***REMOVED***5060708090a0b0c0d0e0f");
			var src:ByteArray = new ByteArray;
			src.writeBytes(pt);
			cbc.encrypt(src);
			assert("CBC_AES256 test 1", Hex.fromArray(src)==Hex.fromArray(ct));
			cbc.decrypt(src);
			assert("CBC_AES256 test 2", Hex.fromArray(src)==Hex.fromArray(pt));
		}
		
		
		/**
		 * For now the main goal is to show we can decrypt what we encrypt in this mode.
		 * Eventually, this should get correlated with some well known vectors.
		 */
		public function testAES():void {
			var keys:Array = [
			"***REMOVED***607080A0B0C0D0***REMOVED***",
			"***REMOVED***A1B1C1E1***REMOVED***242526"];
			var cts:Array = [
			"D8***REMOVED***289EF7D06B506A4FD5BE9C94894C5508A8D8E29AB600DB0261F0555A8FA287B89E65C0973F1F8283E70C72863FE1C8F1***REMOVED***CE05626E961A67B3",
			"59AB30F4D4EE6E4FF9907EF65B1FB68C96890C***REMOVED***B1BE0C93ED51CF21BB5A0101A8C30714EC4F52DBC9C6***REMOVED***7D363F67AB***REMOVED***05E679B68F0B496"];
			var pts:Array = [
			"506812A45F08C889B97***REMOVED***8***REMOVED***6812A45F08C889B97***REMOVED***8***REMOVED***6812A45F08C889B97***REMOVED***8B8359",
			"5C6D71CA30DE8B8***REMOVED***84D2EC7D4B5C6D71CA30DE8B8***REMOVED***84D2EC7D4B5C6D71CA30DE8B8***REMOVED***84D2EC7D4B"];

			for (var i:uint=0;i<keys.length;i++) {
				var key:ByteArray = Hex.toArray(keys[i]);
				var pt:ByteArray = Hex.toArray(pts[i]);
				var aes:AESKey = new AESKey(key);
				var cbc:CBCMode = new CBCMode(aes);
				cbc.IV = Hex.toArray("***REMOVED******REMOVED***0000000000");
				cbc.encrypt(pt);
				var str:String = Hex.fromArray(pt).toUpperCase();
				assert("comparing "+cts[i]+" to "+str, cts[i]==str);
				// back to pt
				cbc.decrypt(pt);
				str = Hex.fromArray(pt).toUpperCase();
				assert("comparing "+pts[i]+" to "+str, pts[i]==str);
			}
		}
		public function testXTea():void {
			var keys:Array=[
			"***REMOVED******REMOVED***0000000000",
			"2b***REMOVED***976775d0e266***REMOVED***"];
			var cts:Array = [
			"2dc7e8d3695b0538d8f1640d46dc***REMOVED***af2ab545e11f3b08e798eb3f17***REMOVED***9d4d20b534aa",
			"790958213819878370eb8251ffda***REMOVED***c5a457fc42502***REMOVED***06fea150be8674c3b8***REMOVED***"];
			var pts:Array=[
			"***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***000000000",
			"74657374206d652***REMOVED***74206d652***REMOVED***74206d652***REMOVED***74206d652e"];
			
			for (var i:uint=0;i<keys.length;i++) {
				var key:ByteArray = Hex.toArray(keys[i]);
				var pt:ByteArray = Hex.toArray(pts[i]);
				var tea:XTeaKey = new XTeaKey(key);
				var cbc:CBCMode = new CBCMode(tea);
				cbc.IV = Hex.toArray("***REMOVED***00000");
				cbc.encrypt(pt);
				var str:String = Hex.fromArray(pt);
				assert("comparing "+cts[i]+" to "+str, cts[i]==str);
				// now go back to plaintext.
				cbc.decrypt(pt);
				str = Hex.fromArray(pt);
				assert("comparing "+pts[i]+" to "+str, pts[i]==str);
			}
		}
	}
}