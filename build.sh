...
function update_nais_settings() {
    ...
    curl -v -s -S --user "${nexusUploader}" --upload-file config/src/main/resources/openam/app-policies.xml "https://repo.adeo.no/repository/raw/nais/sosialhjelp-soknad-api/${versjon}/am/app-policies.xml"
    curl -v -s -S --user "${nexusUploader}" --upload-file config/src/main/resources/openam/not-enforced-urls.txt "https://repo.adeo.no/repository/raw/nais/sosialhjelp-soknad-api/${versjon}/am/not-enforced-urls.txt"
}
